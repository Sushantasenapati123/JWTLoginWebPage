    ...
    public class CodeGenLoginRepository : WizTestRepositoryBase, ICodeGenLoginRepository
    {
         ...
         public async Task<CodeGenLogin> GetLoginDetails(CodeGenLogin log)
        {
            try
            {
                ...
                var results = await Connection.QueryAsync<CodeGenLogin>("Usp_Userelogin", p, commandType: CommandType.StoredProcedure);
        ...
        public async Task<int> ChangePwd(CodeGenLogin Upd)
		{
			try
			{
				...
				var results = await Connection.ExecuteAsync("Usp_Userelogin", p, commandType: CommandType.StoredProcedure);
				return results;
			}
			catch (Exception ex)
			{
